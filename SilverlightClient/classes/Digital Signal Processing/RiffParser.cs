...
namespace RapBattleAudio.classes
{
    ...
    public class RiffParser : IDisposable
    {
        ...
        {
            get { return this.currentChunkOffset + SizeOfRiffChunk; }
        }
        ...
        {
            get { return SizeOfRiffChunk + RiffRound(this.chunk.Size); }
        }
        ...
        public void MoveToNextChunk()
        {
            ...
            Debug.Assert(this.currentChunkOffset > this.containerOffset,
                "The chunk cannot be out of bounds of the container");
            ...
            maxChunkSize = this.containerSize - (this.currentChunkOffset - this.containerOffset);
        ...
        public void PrintChunkInformation(int indent)
        {
            try
            {
                while (true)
                {
                    ...
                    Console.WriteLine("{0} ({1} bytes) {2}", FormatFourCC(this.fccId), this.Chunk.Size,
                        FormatFourCC(this.fccType));
                    if (this.chunk.IsList())
                    {
        ...
        public byte[] ReadDataFromChunk(uint count)
        {
            ...
            this.stream.Position = this.currentChunkOffset + this.chunk.Size - this.bytesRemaining + SizeOfRiffChunk;
