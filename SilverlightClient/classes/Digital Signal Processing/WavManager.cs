...
namespace RapBattleAudio.classes
{
    public class WavManager
    {
        ...
        public static void WriteRawData(Stream rawData, Stream output, uint offset = 0, long maxCount = -1)
        {
            ...
            while ((maxCount != 0) &&
                   (read = rawData.Read(buffer, 0, (maxCount < 0 || maxCount >= 4096) ? 4096 : (int) maxCount)) > 0)
            {
                bwOutput.Write(buffer, 0, read);
                if (maxCount >= 0)
                    maxCount -= read;
                //when maxCount was initially >=0, then after final read this will do maxCount-maxCount=0 and loop will stop when it checks for maxCount!=0
            }
